...
-- siblings
  ...
  CREATE VIEW SELECT student.person_id, COUNT(CASE WHEN sibling.student_id IS NOT NULL THEN 'has_sibling' END) AS number_of_siblings from student LEFT JOIN sibling ON student.person_id = sibling.student_id GROUP BY student.person_id ORDER BY student.person_id;



 -- instructor lessons

 CREATE VIEW SELECT instructor_person_id, count(*) FROM lesson WHERE EXTRACT(YEAR FROM start_time) = '2022' AND EXTRACT(MONTH FROM start_time) = '12' GROUP BY instructor_person_id HAVING COUNT(*) > 0
    ORDER BY count(*) DESC;

-- Show all ensembles in next week
  CREATE VIEW  SELECT lesson_id, to_char(start_time, 'Day') as weekday, genre, start_time,
   CASE 
